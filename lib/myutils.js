...
function expandJSONVar(val, mapping) {
    if (typeof val === 'string') {
        Object.keys(mapping).forEach(function(p) {
...
function expandJSON(templateObj, dictionary) {
    var res = {};
    if (templateObj && typeof templateObj === 'object') {
        var newAttrVal;
        var parsedVal;
        Object.keys(templateObj).forEach(function(key) {
            res = expandJSONVar(templateObj[key], dictionary);
            // if (typeof templateObj[key] === 'object') {
            //     res[expandVar(key, dictionary)] = expandJSON(templateObj[key], dictionary);
            // } else if (typeof templateObj[key] === 'string') {
            //     newAttrVal = expandVar(templateObj[key], dictionary);
            //     // Type transformations
            //     try{
            //         parsedVal = JSON.parse(newAttrVal); // Objects, Numbers, Booleans, nulls...
            //     } catch (e) {
            //         // If the attribute value is not valid json structure, add as String
            //         parsedVal = newAttrVal;
            //     }
            //     res[expandVar(key, dictionary)] = newAttrVal;
            // } else {
            //     res[expandVar(key, dictionary)] = templateObj[key];
            // }
        });
    }
    return res;
