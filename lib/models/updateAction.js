...
function processOptionParams(action, event) {
    ...
    action.parameters.attributes.forEach(function(attr) {
        ...
        // Change String to Number, Booleans and null if necessary
        ...
        } else if (typeof theValue ==='object' || theType !== 'StructuredValue') {
            theValue = myutils.expandJSON(theValue, event);
        }
        ...
        if (theMeta !== undefined) {
            changes[key].metadata = myutils.expandJSON(theMeta, event);
        }
    });
...
function doRequestV2(action, event, callback) {
    ...
    // TODO metadata
    // TODO isPattern, if True, call batchOperaton, else updateEntityAttributes
    // TODO valueTypes (v1 only support Strings, v2 support all JSON types) and complex Values, like objects,arrays,etc
    metrics.IncMetrics(event.service, event.subservice, metrics.actionEntityUpdate);

    var theConnection;
    if (action.parameters.contextBrokerURL !== undefined) {
        // Custom CB
        theConnection = new NGSI.Connection(new URL(action.parameters.contextBrokerURL), {
            'service': event.service,
            'servicepath': event.subservice
        });
    } else {
        // Default CB
        theConnection = new NGSI.Connection(config.orion.URL, {
            'service': event.service,
            'servicepath': event.subservice
        });
    }
    if (action.parameters.updateEntityAttributes !== undefined &&
        typeof action.parameters.updateEntityAttributes === 'object') {

        theConnection.v2.updateEntityAttributes(changes, action.parameters.updateEntityAttributes.options).then(
            (response) => {
                metrics.IncMetrics(event.service, event.subservice, metrics.okActionEntityUpdate);
                alarm.release(alarm.ORION);
        ...
        theConnection.v2.createEntity(changes, {upsert: true}).then(
            (response) => {
                metrics.IncMetrics(event.service, event.subservice, metrics.okActionEntityUpdate);
                alarm.release(alarm.ORION);
                callback(null, response);
    ...
    if (action.parameters.contextBrokerURL !== null) {
        // Close custom CB
        theConnection.close();
    }
}
