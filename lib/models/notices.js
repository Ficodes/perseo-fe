...
function parseLocation2GeoJSON (locStr) {
    var position,
        lat,
        lon;
        ...
        ncr.contextResponses[ix].contextElement.attributes.forEach(function(attr) {
            if (attr.name === 'id') {
                ...
                if (attr.type === 'geo:point') {
                    location = parseLocation2GeoJSON(attr.value);
                }
...
function processCBv2Notice(service, subservice, ncr, ix) {
    ...
    try {
        ...
        for (var attr in attrList) {
            ...
            if (attr !== 'id' && attr !== 'type') {
                ...
                if (attrInfo.type === 'geo:point') {
                    location = parseLocation2GeoJSON(attrInfo.value);
                    n[attr] = location;
                } else if (attrInfo.type === 'DateTime') {
                    date = parseDate(attrInfo.value);
                    n[attr] = parseDate(attrInfo.value);
                } else {
                    n[attr] = attrInfo.value;
                }

                if (attrInfo.metadata) {
                    ...
                    for (var metaKey in attrInfo.metadata) {
                        ...
                        if (attrInfo.metadata[metaKey].type === 'DateTime') {
                            ...
                            metalocation = parseLocation2GeoJSON(attrInfo.metadata[metaKey].value);
        ...
        var localError = new errors.InvalidV2Notice(JSON.stringify(ncr));
        myutils.logErrorIf(localError);
        return localError;
    }
    return n;
...
function DoNotice(orionN, callback) {
    ...
    // Iterate between entities. 1 notice for each entity
...
module.exports.ParseLocation = parseLocation2GeoJSON;
